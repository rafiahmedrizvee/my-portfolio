...
export default function Hero() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from(".hero-title", { y: 60, opacity: 0, delay: 0.2 });
      gsap.from(".hero-text", { y: 40, opacity: 0, delay: 0.4 });
      gsap.from(".hero-btn", { y: 20, opacity: 0, stagger: 0.1, delay: 0.6 });
      gsap.from(".hero-social", { y: 20, opacity: 0, delay: 0.8 });

      gsap.from(imageRef.current, { scale: 0.8, rotate: -8, opacity: 0, duration: 1.4 });
    ...
    >
      ...
      <Particles
        ...
        options={{
          particles: {
            number: { value: 40 },
            color: { value: "#a855f7" },
            links: { enable: true, color: "#a855f7", distance: 150, opacity: 0.2 },
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 items-center w-full">
        ...
        <div className="flex justify-center md:justify-end">
          ...
          >
            <img src={profile} alt="Rafi Ahmed Rizvee" className="w-full h-full object-cover" />
          </div>
        </div>
      </div>
    </section>
